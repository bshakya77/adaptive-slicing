...
from sahi.postprocess.combine import (
    ...
    NMMPostprocess,
    NMSPostprocess,
    OptimizedNMSPostprocess,
    TruncatedNMSPostprocess,
...
POSTPROCESS_NAME_TO_CLASS = {
    ...
    "OptNMS": OptimizedNMSPostprocess,
    "TruncatedNMS": TruncatedNMSPostprocess
}
...
def get_prediction_batched(
    ...
    # Correctly read image using PIL
    if isinstance(image, str):
        print("Image Type 1 ")
        image_as_pil = Image.open(image).convert("RGB")
    ...
    #print("Length of SHIFT AMOUNT: ", shift_amount_list)
    #print("SHIFT AMOUNT: ", shift_amount_list)
    #print("FULL SHAPE: ", full_shape_list)
    ...
    return PredictionResultBatch(
        image, object_prediction_list=object_prediction_list, durations_in_seconds=durations_in_seconds
    )
...
def get_sliced_prediction_batched(
    ...
    time_start = time.time()
    slice_image_result = slice_image_batch(
        image=image,
        slice_height=slice_height,
        slice_width=slice_width,
    ...
    #print("image slice:",slice_image_result.images[0].dtype)
    # perform batch prediction
    #print("SLICED SHIFT AMOUNT: ", slice_image_result.starting_pixels)
    #print("SLICED FULL SHAPE: ", full_shape_list)
    
    prediction_result = get_prediction_batched(
        image=slice_image_result.images[0],
...
def get_batch_predict(
    ...
    image_as_pil = read_image_as_pil(image)
    print("image_as_pil: ", image_as_pil)
    #images_as_pil = [np.ascontiguousarray(read_image_as_pil(img)) for img in image]
    # get prediction
    time_start = time.time()
    detection_model.perform_inference(image_as_pil, num_batch = num_batch)
    time_end = time.time() - time_start
    ...
    # works only with 1 batch  ##TODO: For batch processing
    detection_model.convert_original_predictions(
...
def get_sliced_prediction(
    ...
    postprocess_match_threshold: float = 0.5,
    postprocess_class_agnostic: bool = False,
    postprocess_conf_threshold: float = 0.3,  #added
    postprocess_min_area: float = 1024, #added
    #postprocess_image_height: int = None, #added
    verbose: int = 1,
    ...
    # currently only 1 batch supported  #TODO: batch processing
    num_batch = 1
    # create slices from full image #TOCHECK
    time_start = time.time()
    ...
    time_end = time.time() - time_start
    durations_in_seconds["slice"] = time_end
    print("POST PROCESS: ", postprocess_type)
    print("Confidence Score: ", postprocess_conf_threshold)
    ...
    postprocess = postprocess_constructor(
        ...
        match_metric=postprocess_match_metric,
        class_agnostic=postprocess_class_agnostic,
        conf_threshold = postprocess_conf_threshold,
        min_area = postprocess_min_area,
        #image_height = postprocess_image_height #changes
    )

    # create prediction input
    num_group = int(num_slices / num_batch) #TODO: remove batch processing
    if verbose == 1 or verbose == 2:
        tqdm.write(f"Performing prediction on {num_slices} slices.")
    object_prediction_list = [] #TODO: modify for batch processing
    #print("No of group: ", num_group)
    ...
    for group_ind in range(num_group):
        ...
        #print("IMAGE LIST: ", image_list[0])
        #print("SHIFT AMOUNT: ", shift_amount_list[0])
        # perform batch prediction
        ...
        #here: count the bboxes in each slice prediction
        # convert sliced predictions to full predictions
        for object_prediction in prediction_result.object_prediction_list:
            if object_prediction:  # if not empty
...
def predict(
    ...
    postprocess_conf_threshold: float = 0.3,  #added
    postprocess_min_area: float = 1024, #added
    #postprocess_image_height: int = None,
    novisual: bool = False,
    ...
    **kwargs,
    ...
    # for profiling
    durations_in_seconds = dict()
    print("POST PROCESSING:", postprocess_type)
    ...
    for ind, image_path in enumerate(
        ...
        print("Image Name:", filename_without_extension)
        print("Image Size: ", image_as_pil.size)
        # perform prediction
        if not no_sliced_prediction:
            ...
            prediction_result = get_sliced_prediction(
                ...
                postprocess_conf_threshold = postprocess_conf_threshold,
                postprocess_min_area = postprocess_min_area,
                #postprocess_image_height = postprocess_image_height,
